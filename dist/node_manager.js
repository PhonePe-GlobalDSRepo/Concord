 ...
 */
export function getNewTextNode(text, color = { r: 0.1, g: 0.1, b: 0.1 }, fontName = { family: "Inter", style: "Regular" }) {
 ...
 */
export function getVSpacer(height, color = { r: 0.8, g: 0.8, b: 0.8 }) {
 ...
 */
export function createContainerWithFrames(node1, node2) {
 ...
 */
export function getTokenNode(key, value) {
 ...
 */
export function getTokenFrame(tokens, padding) {
    ...
    applyAutoLayout(tokenFrame, padding, "VERTICAL");
...
export function getSizeFrame(node) {
    let details = [];
    const allValues = Object.keys(Constants.PrimaryDataValues)
        .filter(key => isNaN(Number(key)))
        .map(key => Constants.PrimaryDataValues[key]);
...
export function getSizeDatum(node, dataValueType) {
...
export function appendToDataDetailsFrame(Node, data) {
    applyAutoLayout(Node, Constants.padding2, "VERTICAL");
    let fontName = { family: "Inter", style: "Regular" };
    let datumNodes = [];
    data.forEach((datum) => {
        const datumNode = createContainerWithFrames(getNewTextNode(datum[0], undefined, fontName = fontName), getNewTextNode(datum[1], { r: 0.46, g: 0, b: 0 }, fontName = fontName));
    ...
    datumNodes.forEach((node) => {
        node.resize(Math.max(maxWidth + maxWidth2 + Constants.padding3, 300), node.height);
    });
}
export function getDataDetailsFrame(title, data) {
    ...
    let datumNodes = [];
    data.forEach((datum) => {
        const datumNode = createContainerWithFrames(getNewTextNode(datum[0], undefined, fontName = fontName), getNewTextNode(datum[1], { r: 0.46, g: 0.38, b: 0 }, fontName = fontName));
...
export function createChildTitleNode(child) {
