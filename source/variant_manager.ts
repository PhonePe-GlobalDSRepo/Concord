 ...
 * @returns An array of instances representing each variant of the provided element.
 */
function getVariants(element: InstanceNode): InstanceNode[] {
    ...
    const setNode = element.mainComponent.parent as ComponentSetNode
...
function parseKeyValuePairs(input: string): string[][] {
...
export function generateVariants() {
    ...
    (async () => {
        ...
        const selectedElement: SceneNode | null = figma.currentPage.selection[0];

        // If an element is selected
        if (selectedElement) {
            // Get all variants of the element
            const variants = getVariants(selectedElement as InstanceNode)
            ...
            const instanceNode = selectedElement as InstanceNode
            const setNode = instanceNode.mainComponent?.parent as ComponentSetNode
            ...
            variants.forEach(variant => {
                ...
                let propInfo = parseKeyValuePairs(variant.mainComponent?.name ?? "")
